...
After editing Python files (like `jupyter_projspec/routes.py`):

```bash
python -m py_compile jupyter_projspec/__init__.py  # Check single file for syntax errors
```
...
**Python** (in `jupyter_projspec/*.py` files):
...
- Backend processing in `jupyter_projspec/routes.py`
...
- **✅ Do**: `jupyter_projspec/` with underscores, all lowercase
...
```typescript
const PLUGIN_ID = 'jupyter_projspec:plugin';
```
...
export namespace CommandIDs {
  export const openPanel = 'jupyter_projspec:open-panel';
  export const refreshData = 'jupyter_projspec:refresh-data';
}
...
- Backend routes: `jupyter_projspec/routes.py` (class `DataRouteHandler`)
- Frontend logic: `src/` directory
- Python package: `jupyter_projspec/` directory
...
1. **Read the backend first** — Check `jupyter_projspec/routes.py` to understand the existing API contract
...
Create RESTful endpoints in `jupyter_projspec/routes.py`:
...
def setup_route_handlers(web_app):
    ...
    data_route = url_path_join(base_url, "jupyter_projspec", "data")
...
- Define matching endpoint path strings (e.g., `"hello"`, `"get-data"`) in both `jupyter_projspec/routes.py` and `src/api.ts` to ensure routes sync between backend and frontend
...
2. **Write the code** (TypeScript in `src/`, styles in `style/`, Python in `jupyter_projspec/`)
...
- **Output**: Creates files in `lib/` and `jupyter_projspec/labextension/`
...
- Ask user to search for the extension ID (`jupyter_projspec`) to see if it loaded
...
ls -la jupyter_projspec/labextension/  # Should contain bundled extension
```
...
- Backend routes: `jupyter_projspec/routes.py`
- Server extension config: `jupyter-config/server-config/jupyter_projspec.json`
...
**❌ Don't**: Write frontend interfaces without first checking the backend API contract in `jupyter_projspec/routes.py`
...
**❌ Don't**: Use absolute imports like `from jupyter_projspec.routes import ...`
...
- **Plugin ID** (in `src/index.ts`): `'jupyter_projspec:plugin'`
- **Command IDs** (in `src/commands.ts` or `src/index.ts`): `'jupyter_projspec:command-name'`
...
- **CSS classes** (in `style/index.css`): `.jp-jupyter-projspec-ClassName`
- **API routes** (in `jupyter_projspec/routes.py`): `url_path_join(base_url, "jupyter_projspec", "endpoint")`
